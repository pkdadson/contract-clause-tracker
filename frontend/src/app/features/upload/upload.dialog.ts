...
@Component({
  ...
  template: `
    <div class="fixed inset-0 z-40 bg-ink/40 grid place-items-center"
         (click)="close.emit()">
      <div role="dialog"
           aria-modal="true"
        ...
        <button #dropzone
          ...
          <div class="w-12 h-12 mx-auto rounded-md bg-accent-soft text-accent grid place-items-center mb-3">
          ...
          <div class="text-sm text-ink-muted mt-1">
            or <span class="text-accent underline">click to choose</span> a file
          </div>
...
export class UploadDialog implements AfterViewInit, OnDestroy {
  ...
  private processFile(file: File): void {
    ...
    if (file.size > MAX_BYTES) {
      this.error.set(
        `Files must be under 5 MB (got ${(file.size / 1024 / 1024).toFixed(1)} MB)`,
      );
      return;
    }
